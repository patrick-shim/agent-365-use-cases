...
if (-not $a365Command) {
    Write-Host "a365 CLI not found — skipping bearer token refresh. Install with: dotnet tool install --global Microsoft.Agents.A365.DevTools.Cli"
    exit 0
}
...
$appId = if ($appIdLine) { ($appIdLine -split '=', 2)[1].Trim() } else { $null }

if ([string]::IsNullOrWhiteSpace($appId)) {
    Write-Host "CLIENT_APP_ID is not set — skipping bearer token refresh. MCP tool auth will be unavailable."
    exit 0
}
