...
# --- Helper Functions ---
async def safe_send_activity(context: TurnContext, message_or_activity):
    """
    Safely send an activity with error handling for playground compatibility.
    Swallows errors from the playground's mock connector.
    """
    try:
        await context.send_activity(message_or_activity)
    except Exception as e:
        # Log but don't crash - playground connector may reject some activities
        logger.debug(f"Activity send failed (non-critical): {e}")
...
class GenericAgentHost:
    ...
    async def _validate_agent_and_setup_context(self, context: TurnContext):
        ...
        if not self.agent_instance:
            ...
            await safe_send_activity(context, "❌ Sorry, the agent is not available.")
            return None
    ...
    def _setup_handlers(self):
        ...
        async def on_installation_update(context: TurnContext, _: TurnState):
            ...
            if action == "add":
                await safe_send_activity(context, "Thank you for hiring me! Looking forward to assisting you in your professional journey!")
            elif action == "remove":
                await safe_send_activity(context, "Thank you for your time, I enjoyed working with you.")
        ...
        async def on_message(context: TurnContext, _: TurnState):
            try:
                ...
                with BaggageBuilder().tenant_id(tenant_id).agent_id(agent_id).build():
                    ...
                    await safe_send_activity(context, "Got it — working on it…")
                    await safe_send_activity(context, Activity(type="typing"))
                ...
                await safe_send_activity(context, f"Sorry, I encountered an error: {str(e)}")
            ...
            try:
                ...
                with BaggageBuilder().tenant_id(tenant_id).agent_id(agent_id).build():
                    ...
                    if notification_activity.notification_type == NotificationTypes.EMAIL_NOTIFICATION:
                        ...
                        await safe_send_activity(context, response_activity)
                        return
